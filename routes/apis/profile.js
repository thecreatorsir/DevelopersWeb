...
router.get("/user/:user_id", (req, res) => {
  ...
  Profile.findOne({ user: req.params.user_id })
    ...
    .catch((err) =>
      res.status(404).json({ noprofile: "No profile is found for this user" })
    );
});
...
router.post(
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then((profile) => {
      ...
      //update experience part of peofile
      //push new experience at front
...
// @desc    DELETE perticular experience from the profile
// @access  Private
...
// @route   POST api/profile/eduaction/:edu_id
// @desc    DELETE perticular education from the profile
// @access  Private
